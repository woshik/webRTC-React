...
const ChatModerator = (props) => {
  ...
  const { roomClient, isChatModerator, room, classes } = props;

  if (!isChatModerator) return null;

  return (
    <ul className={classes.root}>
      <li className={classes.listheader}>
        <FormattedMessage id="room.moderatoractions" defaultMessage="Moderator actions" />
      </li>
      <Button
...
export default withRoomContext(
  connect(makeMapStateToProps, null, null, {
    areStatesEqual: (next, prev) => {
      return prev.room === next.room && prev.me === next.me && prev.peers === next.peers;
    },
