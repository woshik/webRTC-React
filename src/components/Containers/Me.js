...
import { meProducersSelector, makePermissionSelector } from '../Selectors';
...
const Me = (props) => {
  ...
  const videoVisible = Boolean(webcamProducer) && !webcamProducer.locallyPaused && !webcamProducer.remotelyPaused;

  const screenVisible = Boolean(screenProducer) && !screenProducer.locallyPaused && !screenProducer.remotelyPaused;
  ...
  const [screenShareTooltipOpen, screenShareTooltipSetOpen] = React.useState(false);
  ...
  if (micProducer && micProducer.score) {
    audioScore = micProducer.score.reduce((prev, curr) => (prev.score < curr.score ? prev : curr));
  }

  let videoScore = null;

  if (webcamProducer && webcamProducer.score) {
    videoScore = webcamProducer.score.reduce((prev, curr) => (prev.score < curr.score ? prev : curr));
  }
      ...
      >
        {me.browser.platform !== 'mobile' && smallContainer && (
          <div className={classnames(classes.ptt, micState === 'muted' && me.isSpeaking ? 'enabled' : null)}>
            <FormattedMessage id="me.mutedPTT" defaultMessage="You are muted, hold down SPACE-BAR to talk" />
          </div>
        )}
        <div className={classes.viewContainer}>
          ...
          <p className={classnames(classes.meTag, hover ? 'hover' : null, smallContainer ? 'smallContainer' : null)}>
            <FormattedMessage id="room.me" defaultMessage="ME" />
          </p>
          {!settings.buttonControlBar && (
            <div
              className={classnames(classes.controls, settings.hiddenControls ? 'hide' : null, hover ? 'hover' : null)}
              onMouseOver={() => setHover(true)}
              onMouseOut={() => setHover(false)}
              onTouchStart={() => {
                if (touchTimeout) clearTimeout(touchTimeout);
                  ...
                  {smallContainer ? (
                    <div>
                      <IconButton
                        ...
                        size="small"
                        onClick={() => {
                          if (micState === 'off') roomClient.updateMic({ start: true });
                          else if (micState === 'on') roomClient.muteMic();
                          else roomClient.unmuteMic();
                        }}
                      >
                        {micState === 'on' ? (
                          <MicIcon color={me.isAutoMuted ? 'secondary' : 'primary'} style={{ opacity: noiseVolume }} />
                        ) : (
                          <MicOffIcon />
                        )}
                      </IconButton>
                      ...
                      >
                        {micState === 'on' ? (
                          <MicIcon
                            ...
                            style={
                              me.isAutoMuted && settings.voiceActivatedUnmute
                                ? { opacity: noiseVolume }
                                : { opacity: 1 }
                            }
                          />
                  ...
                  {smallContainer ? (
                    <div>
                      <IconButton
                        ...
                        size="small"
                        onClick={() => {
                          webcamState === 'on' ? roomClient.disableWebcam() : roomClient.updateWebcam({ start: true });
                        }}
                      >
        ...
        return (
          ...
          >
            <div className={classes.viewContainer} style={style}>
              <p
                className={classnames(classes.meTag, hover ? 'hover' : null, smallContainer ? 'smallContainer' : null)}
              >
                <FormattedMessage id="room.me" defaultMessage="ME" />
              </p>
              <div
...
const makeMapStateToProps = () => {
  ...
  const mapStateToProps = (state) => {
    ...
    if (state.peerVolumes[state.me.id] < state.settings.noiseThreshold) {
      ...
      volume =
        1 +
        Math.abs(state.peerVolumes[state.me.id] - state.settings.noiseThreshold) /
  ...
  connect(makeMapStateToProps, null, null, {
    areStatesEqual: (next, prev) => {
      return (
        prev.room === next.room &&
        prev.me === next.me &&
        Math.round(prev.peerVolumes[prev.me.id]) === Math.round(next.peerVolumes[next.me.id]) &&
