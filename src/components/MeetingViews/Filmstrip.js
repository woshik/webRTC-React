...
class Filmstrip extends React.PureComponent {
  ...
  isSharingCamera = (peerId) =>
    ...
    this.props.peers[peerId].consumers.some((consumer) => this.props.consumers[consumer].source === 'screen');
  ...
  updateDimensions = () => {
    ...
    const availableSpeakerHeight =
      root.clientHeight * 0.8 - (this.props.toolbarsVisible || this.props.permanentTopBar ? PADDING_V : PADDING_H);
    ...
    if (filmStrip) {
      ...
      if (filmStripWidth * this.props.boxes > availableWidth - FILMSTRING_PADDING_H) {
        filmStripWidth = (availableWidth - FILMSTRING_PADDING_H) / this.props.boxes;
  ...
  componentDidUpdate(prevProps) {
    if (prevProps !== this.props) {
      if (this.props.activeSpeakerId != null && this.props.activeSpeakerId !== this.props.myId) {
  ...
  render() {
    const { roomClient, peers, myId, advancedMode, spotlights, toolbarsVisible, permanentTopBar, classes } = this.props;
    ...
    return (
      ...
      >
        <div className={classes.speaker} ref={this.activePeerContainer}>
          {peers[activePeerId] && <SpeakerPeer advancedMode={advancedMode} id={activePeerId} style={speakerStyle} />}
        </div>
        ...
        <div className={classes.filmStrip} ref={this.filmStripContainer}>
          <Grid container justify="center" spacing={0}>
            <Grid item>
              ...
              >
                <Me advancedMode={advancedMode} style={peerStyle} smallContainer />
              </div>
            </Grid>
              ...
              if (spotlights.find((spotlightsElement) => spotlightsElement === peerId)) {
                return (
                  <Grid key={peerId} item>
                    ...
                    >
                      <Peer advancedMode={advancedMode} id={peerId} style={peerStyle} smallContainer />
                    </div>
                  </Grid>
                );
              } else {
