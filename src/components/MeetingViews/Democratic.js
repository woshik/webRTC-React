...
import { spotlightPeersSelector, videoBoxesSelector } from '../Selectors';
...
class Democratic extends React.PureComponent {
  ...
  updateDimensions = () => {
    ...
    const width = this.peersRef.current.clientWidth - (this.props.buttonControlBar ? PADDING : 0);
    const height =
      this.peersRef.current.clientHeight - (this.props.toolbarsVisible || this.props.permanentTopBar ? PADDING : 0);
  ...
  componentDidUpdate(prevProps) {
    if (prevProps !== this.props) this.updateDimensions();
  }

  render() {
    const { advancedMode, spotlightsPeers, toolbarsVisible, permanentTopBar, buttonControlBar, classes } = this.props;
